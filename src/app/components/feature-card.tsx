...
import { ChartNoAxesCombined, DatabaseZap, LayoutDashboard } from "lucide-react";
...
export default function FeatureCard({ feature }) {
  ...
  const Icon = featureCardIconMap[feature.icon];
  ...
  return (
    <motion.div initial={{ y: 20, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.25 }} className="relative rounded-xl border border-white/30 px-6 py-8">
      <motion.div
        ref={borderElement}
      ...
      />
      <Card key={feature.id} className="border-none bg-black p-6 text-center text-white sm:flex-1">
        ...
        <CardContent>
          ...
          <p className="md:text-md mt-2 text-white/70 sm:text-sm">{feature.description}</p>
        </CardContent>
      </Card>
    </motion.div>
  );
